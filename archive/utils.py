...
#TODO add flow warpping technique for semantic images
def sem_flow_warp(src_img, flow, nn_warp):
  """ inverse warp a source image to the target image plane based on flow field
  Args:
    src_img: the source  image [batch, height_s, width_s, 3]
    flow: target image to source image flow [batch, height_t, width_t, 2]
  ...
  src_pixel_coords = tgt_pixel_coords + flow
  if nn_warp:
    output_img = nearest_neighbor_sampler(src_img, src_pixel_coords)
  else:
    output_img = bilinear_sampler(src_img, src_pixel_coords)
  return output_img
...
#TODO nearest_neighbor_sampler
def nearest_neighbor_sampler(imgs, coords):
  """Construct a new image by nearest neighbor sampling from the input image.
  ...
  with tf.name_scope('image_sampling'):
    ...
    w00 = tf.cast(wt_x0 <= wt_x1, 'float32')*tf.cast(wt_y0 <= wt_y1, 'float32') #wt_x0 * wt_y0
    w01 = tf.cast(wt_x0 <= wt_x1, 'float32')*tf.cast(wt_y0 > wt_y1, 'float32')#wt_x0 * wt_y1
    w10 = tf.cast(wt_x0 > wt_x1, 'float32')*tf.cast(wt_y0 <= wt_y1, 'float32')#wt_x1 * wt_y0
    w11 = tf.cast(wt_x0 > wt_x1, 'float32')*tf.cast(wt_y0 > wt_y1, 'float32')#wt_x1 * wt_y1
